    ...
    final Runnable rAfterDownload = new Runnable() {
        ...
        public void run() {
            ...
            if (BgJobService.isActive()) {
                ...
                tc.getPref(R.string.Layers_pref_chooseMap2_key, "").setValue("MAPSFORGE: germany_oam.osm.map");
...
//                }
    ...
    protected void setup() {
        ...
        tc.timer.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    intent.setData(Uri.parse("mf-v4-map://download.openandromaps.org/mapsV4/Germany/Germany.zip"));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
    ...
    protected void addRegexs() {
        addRegex("Testcase.start.*  TestInitD");
